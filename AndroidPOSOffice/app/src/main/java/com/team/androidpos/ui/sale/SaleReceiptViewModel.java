...
public class SaleReceiptViewModel extends AndroidViewModel {
    ...
    public LiveData<Sale> sale = Transformations.switchMap(saleId, id -> {
        return repo.getSale(id);
    });

    public final LiveData<List<SaleProduct>> saleProductLiveData = Transformations.switchMap(saleId, id -> repo.getSaleProduct(id));
